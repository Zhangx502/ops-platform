...
### 3.1 基础环境安装（三台虚拟机）
1. 安装 JDK 8：`yum install -y java-1.8.0-openjdk-devel`
2. 安装 Nginx（仅 V1）：`yum install -y epel-release` 和 `yum install -y nginx`
3. 安装 MySQL 5.7（V2/V3）：
...
1. 编辑 V2 的 `/etc/my.cnf`，在 `[mysqld]` 段添加：`server-id=1` 和 `log-bin=mysql-bin`
2. 编辑 V3 的 `/etc/my.cnf`，在 `[mysqld]` 段添加：`server-id=2`
3. 在 V2 上创建复制用户：`CREATE USER 'repl_user'@'192.168.205.12' IDENTIFIED BY 'ReplPass123!';` 和 `GRANT REPLICATION SLAVE ON *.* TO 'repl_user'@'192.168.205.12';`
4. 在 V3 上启动同步：`CHANGE MASTER TO MASTER_HOST='192.168.205.11', MASTER_USER='repl_user', MASTER_PASSWORD='ReplPass123!', MASTER_LOG_FILE='mysql-bin.000001', MASTER_LOG_POS=154;` 和 `START SLAVE;`
...
4. 错误码 13 在 Linux 中表示 EACCES，锁定 SELinux

**解决方案**：
1. 执行 `setsebool -P httpd_can_network_connect 1`
...
### 迁移背景
若依原本使用 V1 本地 MySQL，为与主从复制架构统一，将数据库迁移至 V2 主库。

### 迁移步骤
1. V1 导出数据库：`mysqldump -u root -proot ry > /tmp/ry_backup.sql`
2. 使用 `scp` 将备份文件传输至 V2
3. V2 创建 `ry` 库并导入备份：`mysql -u root -proot ry < /tmp/ry_backup.sql`
4. 修改若依 `application-druid.yml` 中数据库连接为 `jdbc:mysql://192.168.205.11:3306/ry`
5. 重新打包部署，重启若依

### 迁移验证
1. 关闭 V1 MySQL 服务，若依仍正常运行
2. V2 上查询 `sys_user` 表有若依用户数据
3. V3 从库同步正常，`Slave_IO_Running` 和 `Slave_SQL_Running` 均为 Yes
